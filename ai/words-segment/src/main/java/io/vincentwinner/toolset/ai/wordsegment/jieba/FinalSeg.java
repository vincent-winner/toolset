...
public class FinalSeg {
    ...
    private static final class FinalSegInstance {
        private static final FinalSeg INSTANCE = new FinalSeg();
    }

    protected static FinalSeg getInstance() {
        return FinalSegInstance.INSTANCE;
    }
        ...
        try (InputStream is = this.getClass().getResourceAsStream(PROB_EMIT);){
            ...
            while (br.ready()) {
                ...
                if (tokens.length == 1) {
                    ...
                    Objects.requireNonNull(values).put(tokens[0].charAt(0), Double.valueOf(tokens[1]));
                }
            }
        } catch (IOException e) {
    ...
    protected void cut(String sentence, List<String> tokens) {
    ...
    protected void viterbi(String sentence, List<String> tokens) {
        ...
        for (int i = 1; i < sentence.length(); ++i) {
            ...
            for (char y : states) {
                ...
                if (emp == null)
                    emp = MIN_FLOAT;
                JiebaPair<Character> candidate = null;
                for (char y0 : prevStatus.get(y)) {
                    ...
                    if (null == candidate)
                        candidate = new JiebaPair<Character>(y0, tranp);
                    else if (candidate.freq <= tranp) {
                        candidate.freq = tranp;
                        candidate.key = y0;
                    }
                }
                vv.put(y, Objects.requireNonNull(candidate).freq);
