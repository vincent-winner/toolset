...
public class JiebaSegmenter {
    ...
    private static final FinalSeg finalSeg = FinalSeg.getInstance();
    ...
    private Map<Integer, JiebaPair<Integer>> calc(String sentence, Map<Integer, List<Integer>> dag) {
        int N = sentence.length();
        HashMap<Integer, JiebaPair<Integer>> route = new HashMap<Integer, JiebaPair<Integer>>();
        route.put(N, new JiebaPair<Integer>(0, 0.0));
        for (int i = N - 1; i > -1; i--) {
            JiebaPair<Integer> candidate = null;
            for (Integer x : dag.get(i)) {
                double freq = wordDict.getFreq(sentence.substring(i, x + 1)) + route.get(x + 1).freq;
                if (null == candidate) {
                    candidate = new JiebaPair<Integer>(x, freq);
    ...
    public List<String> sentenceProcess(String sentence) {
        ...
        Map<Integer, JiebaPair<Integer>> route = calc(sentence, dag);
